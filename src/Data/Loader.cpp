...
void Loader::_loadModules() {
  ...
  for (const auto &file_iterator :
       QDirListing(module_path, QDirListing::IteratorFlag::FilesOnly)) {
    try {
      spdlog::info("loading: {}", file_iterator.fileName().toStdString());
      std::fstream file(file_iterator.absoluteFilePath().toStdString(),
                        std::fstream::in);
      if (!file.is_open())
        throw std::runtime_error("Failed to open file " +
                                 file_iterator.fileName().toStdString());
      if (file.bad()) {
      ...
      spdlog::error("could not parse {} : {}",
                    file_iterator.absoluteFilePath().toStdString(), e.what());
    }
  }
}
...
void Loader::_loadWorkforce() {
  ...
  try {
    ...
    if (!file.is_open())
      throw std::runtime_error("Failed to open file " +
                               workforce_path.toStdString());
    if (file.bad())
      throw std::runtime_error("Failed to read file " +
                               workforce_path.toStdString());

    auto j = nlohmann::json::parse(file);
    this->_workforce = j.get<
        std::map<std::string, std::vector<std::pair<t_ware_id, double>>>>();
    file.close();
