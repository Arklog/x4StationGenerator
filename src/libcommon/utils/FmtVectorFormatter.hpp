...
template<>
struct fmt::formatter<common::types::WareAmount> {
    constexpr auto parse(fmt::format_parse_context &ctx) -> fmt::format_parse_context::iterator {
        return ctx.begin();
    }

    template<typename FormatContext>
    auto format(const common::types::WareAmount &ware_amount,
                FormatContext &                  ctx) const -> typename FormatContext::iterator {
        return fmt::format_to(ctx.out(), "({}, {})", ware_amount.id, ware_amount.amount);
    }
};
