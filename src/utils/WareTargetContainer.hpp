...
public:
   ...
   * @return
   */
  [[nodiscard]] const WareTarget *getTarget(const t_ware_id &ware_id) const;

  const std::vector<WareTarget *> &getPrimaryTargets();
  const std::vector<WareTarget *> &getSecondaryTargets();
};
