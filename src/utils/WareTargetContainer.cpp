...
const WareTarget *
WareTargetContainer::getTarget(const t_ware_id &ware_id) const {
  const auto iter =
      std::ranges::find_if(ware_targets, [ware_id](const WareTarget &target) {
        return target.ware_id == ware_id;
      });
...
const std::vector<WareTarget *> &WareTargetContainer::getPrimaryTargets() {
  return ware_targets_primary;
}

const std::vector<WareTarget *> &WareTargetContainer::getSecondaryTargets() {
