...
SummarySection::SummarySection (const Store &store, QWidget *parent)
    : QWidget (parent), ui (new Ui::SummarySection), store_ (store)
{
    ui->setupUi (this);
...
void SummarySection::updateCostTab (const t_module_quantity &modules)
{
    ...
    {
	const auto &module = store_.modules.by_id.at (module_id);
	...
	for (const auto &ware : wares)
	{
		...
		store_.wares.by_id.at (ware.id)->name);
	...
	for (const auto &[module_id, module_amount] : modules)
	{
	    auto module = store_.modules.by_id.at (module_id);
