...
WareSelectionSection::WareSelectionSection (Settings &settings,
    ...
    : QWidget (parent), ui (new Ui::WareSelectionSection), settings_{settings},
      store_ (store)
{
    ui->setupUi (this);
    ...
    auto const &modules = store.modules.all;
    for (auto const &habitat : store.modules.habitats)
    {
	ui->habitat_input->addItem (QString::fromStdString (habitat->name));
    }
