...
WareConfigurator::WareConfigurator(const t_ware_id &ware_id, QWidget *parent)
  ...
  const auto &production_methods = getModules(ware_id);

  ui->ware_label->setText(QString(ware_name.c_str()));

  this->ware_target.source_module = production_methods.begin()->first;
  ...
  this->ware_target.source_module = getProductionMethodFromName(
  ...
  connect(ui->production_method_combo_box, &QComboBox::currentTextChanged,
          trigger_update_production_method);
